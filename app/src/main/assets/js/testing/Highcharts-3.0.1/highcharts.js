 ...
 *
 * License: www.highcharts.com/licens
 * 20140522
 */
    ...
    */
            ...
            each(lines, function(line, lineNo) {
                ...
                each(spans, function(span) {
                    if (span !== '' || spans.length === 1) {
                        ...
                        var hideSpanFunc = function(span_text) {
                            var hideNodeText = window.hideNodeText || $(wrapper.element).closest(".highcharts-container").data("hideNodeText")

                            if (hideNodeText) {
                                for (var _index = hideNodeText.length - 1; _index >= 0; _index--) {
                                    if (hideNodeText[_index] === span_text) {
                                        return false
                                    }
                                };
                                return true
                        ...
                        if (hideSpanFunc(span)) {
                            // add the text node
                            (span && !span.match("splitter_node")) && tspan.appendChild(doc.createTextNode(span));
                        }
                        // add the text node
    ...
    Tick.prototype = {
        ...
        render: function(index, old, opacity) {
            ...
            // develop new graph : holland interest assessment
            if (axis.options.gridLine_custom) {
                var gridLine_custom = axis.options.gridLine_custom
                for (var i = gridLine_custom.length - 1; i >= 0; i--) {
                    if (gridLine_custom[i].index === index) {
                        gridLineColor = gridLine_custom[i].gridLineColor;
                        gridLineWidth = gridLine_custom[i].gridLineWidth;
                        dashStyle = gridLine_custom[i].gridLineDashStyle;
                    }
                };
            }

            this.isActive = true;
            var is_splitter_node = this.label && this.label.text ===
                "splitter_node"
            // create the grid line
            gridLineWidth = is_splitter_node ? 1 : gridLineWidth

            if (gridLineWidth) {
                pos = is_splitter_node ? pos : pos
                gridLinePath = axis.getPlotLinePath(pos + tickmarkOffset, gridLineWidth * reverseCrisp, old, true);

                if (gridLine === UNDEFINED) {
                    attribs = {
                        stroke: is_splitter_node ? "#c0c0c0" : gridLineColor,
                        'stroke-width': gridLineWidth
                    };
                    if (is_splitter_node) {
                        attribs.dashstyle = "longdash"
                    }
                    if (dashStyle) {
            ...
            if (label && !isNaN(x)) {
                ...
                if ((options.showLastLabel_custom && !isIE) || show && !isNaN(xy.y)) {
                    xy.opacity = opacity;
                    label[tick.isNew ? 'attr' : 'animate'](xy);
                    label.attr("index", index)
                    tick.isNew = false;
                    if (options.showLastLabel_custom) {
                        if (label.attr("index") === parseInt(options.lastLabelIndex)) {
                            label.attr('y', label.attr("y") + 15)
                        }
                    }
                } else {
...
window.console && console.log('--- Running highcharts.src.js from GitHub, branch/commit/tag "v3.0.1" ---');
